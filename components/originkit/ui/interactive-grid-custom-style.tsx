...
interface ImageItem {
  src: string;
  /** Adicionado por nos: transforma a celula em link. */
  href?: string;
  /** Adicionado por nos: texto sob o icone. */
  label?: string;
}
...
interface InteractiveGridProps {
  ...
  rotateX: number;
  rotateY: number;
  /** Adicionado por nos: mostra o texto sob o icone. */
  showLabels?: boolean;
  style?: CSSProperties;
}
...
function __OriginkitBase_InteractiveGrid(props: Partial<InteractiveGridProps>) {
  const {
  ...
  const items = useMemo(() => {
    const list = (images ?? [])
      .map((image): ImageItem => (typeof image === "string" ? { src: image } : image))
...
export default function InteractiveGrid(props: Record<string, unknown>) {
  return <__OriginkitBase_InteractiveGrid {...(__originkitPresetProps as Record<string, unknown>)} {...props} />;
}